...
static void test_012(const size_t testNum, TestInput &input) {
  ...
  ASSERT_TRUE(j->get_node_type_by_path("obj/testNum") ==
              json::JsonDataType::JDT_INTEGER);
...
static void test_013(const size_t testNum, TestInput &input) {
  {
    std::string s =
        R"({ "wrap" : { "a":null, "b": 999, "c" : [1, 2, 3], obj: {"testNum":42}}})";
    auto j = json::parse(s);
    ...
    ASSERT_TRUE(
        std::dynamic_pointer_cast<json::node_integer>(arr->data[0])->data == 1);
    ASSERT_TRUE(
        std::dynamic_pointer_cast<json::node_integer>(arr->data[1])->data == 2);
    ASSERT_TRUE(
        std::dynamic_pointer_cast<json::node_integer>(arr->data[2])->data == 3);
  }
}
...
{ "a" : [ 1, 2, 3 ] }
  ...
  ASSERT_TRUE(std::dynamic_pointer_cast<json::node_integer>(a->data[0])->data ==
              0);
  ASSERT_TRUE(std::dynamic_pointer_cast<json::node_integer>(a->data[4])->data ==
              4);
