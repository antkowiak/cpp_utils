...
public:
  ...
  Statemachine() : Statemachine(getDefaultInitialState()) {}
  ...
  void print() const {
    ...
    if (!m_transitionQueue.empty()) {
      std::cout << "next transition: " << m_transitionQueue.front()
                << std::endl;
    }
  ...
  bool runOneState() {
    ...
    if (m_transitionFunctions.find(transitionToFind) ==
        m_transitionFunctions.end()) {
      return false;
    }
