...
template <typename T> bool contains(const std::list<T> &lst, const T &element) {
...
template <typename T> T max3(const T &t1, const T &t2, const T &t3) {
  return std::max(t1, std::max(t2, t3));
}

// Return the minimum of three values
template <typename T> T min3(const T &t1, const T &t2, const T &t3) {
...
static bool is_quote(const char c) { return (c == '"'); }

// Returns true if the provided character is the equals sign
static bool is_equals(const char c) { return (c == '='); }
