...
public:
  ...
  get_children_by_name(const std::string &child_name) const {
  ...
  // populates a vector of nodes with nodes that match the provided full path
  // (path specified by '/' delimited string
  void find_nodes_by_path(const std::string &path,
                          std::vector<std::shared_ptr<node>> &output) const {
    std::vector<std::string> split_path =
        algorithm_rda::split_string_to_vector(path, "/");
    find_nodes_by_path(split_path, output);
  }

  // populates a vector of nodes with nodes that math the provided full path
  // (path specified by vector)
  void find_nodes_by_path(const std::vector<std::string> &path,
                          std::vector<std::shared_ptr<node>> &output) const {
    if (path.empty()) {
      return;
    } else if (path.size() == 1) {
  ...
  // populates a vector of {path, node} pairs that match all sub-child nodes
  // that have the given name
  void find_nodes_by_name(
      const std::string &node_name,
      std::vector<std::pair<std::string, std::shared_ptr<xml::node>>> &output)
      const {
    find_nodes_by_name(node_name, "", output);
  }

  // populates a vector of {path, node} pairs that match all sub-child nodes
  // that have the given data
  void find_nodes_by_data(
...
protected:
  ...
  // that have the given name
  void find_nodes_by_name(
      const std::string &node_name, std::string cur_path,
      std::vector<std::pair<std::string, std::shared_ptr<xml::node>>> &output)
      const {
    if (cur_path.empty())
      cur_path += name;
  ...
  // that have the given data
  void find_nodes_by_data(
      const std::string &data_value, std::string cur_path,
      std::vector<std::pair<std::string, std::shared_ptr<xml::node>>> &output)
      const {
    if (cur_path.empty())
      cur_path += name;
...
public:
  ...
  document(const std::string &input) { parse(input); }

  // destructor
  virtual ~document() = default;

  // returns the header node of the document
  std::shared_ptr<node> get_header() const { return header; }
...
protected:
  ...
  void parse_recursive(std::shared_ptr<node> parent, const std::string &input,
                       size_t &index) {
    while (index < input.size()) {
      ...
      if (is_closing) {
        ...
        if (is_tag_data_to_read(input, index)) {
          tag->data =
              algorithm_rda::string_index_utils::read_and_advance_until_next(
                  input, "<", index);
          algorithm_rda::string_index_utils::
              strip_leading_and_trailing_whitespace(tag->data);
        }
  ...
  void
  to_string_recursive(std::stringstream &ss, std::shared_ptr<node> tag,
                      const size_t indent,
                      const size_t indent_size = DEFAULT_XML_INDENT) const {
    // compute the indent string to be "indent_size" number of spaces
    const std::string indent_str =
        algorithm_rda::string_index_utils::string_indent(" ", indent_size);

    // add any comments to the string
    for (auto comment : tag->comments) {
      ss << algorithm_rda::string_index_utils::string_indent(indent_str,
                                                             indent);
      ss << "<!--" << comment << "-->" << std::endl;
    }
  ...
  std::shared_ptr<node> create_node(const std::string &input) const {
    ...
    algorithm_rda::string_index_utils::advance_index_past_all(
        tag_text, start, tag_text.size(),
        algorithm_rda::string_index_utils::WHITESPACE_CHARS);

    size_t end = start;

    // find the end of the "name" by reading until end of string or until
    // whitespace is encountered
    algorithm_rda::string_index_utils::advance_index_past_all_not(
        tag_text, end, tag_text.size(),
        algorithm_rda::string_index_utils::WHITESPACE_CHARS);
    ...
    // true if we are reading the key. false if we are looking to read the
    // value.
    ...
    for (; cur < tag_text.size(); ++cur) {
      ...
      // if whitespace is encountered
        ...
        if (in_quote)
          ...
          // if not inside of quoted string, we must have reached the end of the
          // value text, so prepare to read the next key
          reading_key = true;
  ...
  std::string read_header_text(const std::string &input, size_t &index) const {
    if (!contains_header(input)) {
      ...
      algorithm_rda::string_index_utils::advance_index_past_next(
  ...
  bool is_closing_tag(const std::string &input,
                      const size_t start_index) const {
    return algorithm_rda::string_index_utils::string_starts_with(input, "/",
                                                                 start_index);
  }
  ...
  // Returns true if there is tag data to read (as opposed to further nested
  // tags.) Ex: <Blah>some data</Blah> vs <Blah><Child>some data</Child></Blah>
  bool is_tag_data_to_read(const std::string &input, size_t index) const {
    // read (starting at index) for the next non-whitespace char.  If it isn't
    // "<" then it is data
    while (index < input.size()) {
