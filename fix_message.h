...
public:
  fix_message() : m_input(nullptr), m_buffer(nullptr), m_allocatedMem(false) {}

  fix_message(const fix_message &) = delete;
  fix_message &operator=(const fix_message &) = delete;

  virtual ~fix_message() { clear(); }
  ...
  virtual void init(const std::string &input) { init(input.c_str()); }
...
// memory than the "map" implementation.
